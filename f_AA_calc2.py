...
sT3_raster_model = TD_data_mu0[:, 1]/TD_data_mu0[:, 0]**3.0
...
sT3_raster_model = sT3_raster_model[:cutoff_ind]
...
sT3_raster_model = sT3_raster_model[:-cutoff_ind_2]
...
for i in range(0, len(phi0_raster)):
    ...
    V_part_raster[i] = (V(phi_H, *V_args)/-12.0)**(1.0/2.0)/(dV_dphi(phi_H, *V_args)/-12.0)*np.exp(2.0/3.0*integral_raster[i])*phi_H**(-8.0/mq)
...
sT3_m_tck = splrep(T_raster_model[::-1], sT3_raster_model[::-1])

chi2hat_sT3_ratio = chi2hat_mu0(T_raster_model, mu0_chi2hat_tck)/sT3_mu0(T_raster_model, mu0_sT3_tck)
#chi2hat_sT3_ratio = chi2hat_mu0(T_raster_model, mu0_chi2hat_tck)/sT3_mu0(T_raster_model, sT3_m_tck)
#chi2hat_sT3_ratio = chi2hat_mu0(T_raster_model, mu0_chi2hat_tck)/sigma_AA_raster
figure(12)
plot(T_raster_model, chi2hat_mu0(T_raster_model, mu0_chi2hat_tck))
plot(T_raster_model, sT3_mu0(T_raster_model, mu0_sT3_tck))
plot(T_raster_model, sT3_mu0(T_raster_model, sT3_m_tck))
# figure(13)
...
f_AA_raster = (dsT3_chi2hat_ratio_dphi0/V_part_raster)**(-1)

figure(16)
plot(phi0_raster, V_part_raster)
